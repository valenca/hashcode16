def greedy(s, v, offset_x, offset_y):
    #s = 9
    ...
    if fit[0][1] == 0:
            ...
            for i in range(s):
                    x,y = rBounds[i]
                    if y != -1:
                            print("PAINT_LINE %d %d %d %d" % (i + offset_x, x + offset_y, i + offset_x, y + offset_y))
                            for j in range(x, y+1):
                                    if m[i][j] == 0:
                                            print("ERASE_CELL %d %d" % (i + offset_x, j + offset_y))
    elif fit[0][1] == 2:
            for j in range(s):
                    x,y = cBounds[j]
                    if y != -1:
                            print("PAINT_LINE %d %d %d %d" % (x + offset_x, j + offset_y, y + offset_x, j + offset_y))
                            for i in range(x, y+1):
                                    if m[i][j] == 0:
                                            print("ERASE_CELL %d %d" % (i + offset_x, j + offset_y))
    elif fit[0][1] == 3:
            for r,c in v:
                    print("PAINT_LINE %d %d %d %d" % (r + offset_x, c + offset_y, r + offset_x, c + offset_y))
    elif fit[0][1] == 4:
            for a,b,c,d in rSplits:
                    print("PAINT_LINE %d %d %d %d" % (a + offset_x, b + offset_y, c + offset_x, d + offset_y))
    elif fit[0][1] == 5:
